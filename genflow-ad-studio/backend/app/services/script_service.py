...
# CymbalWealth allowed product categories — only these may be used as ad subjects.
_ALLOWED_CATEGORIES = {
    ...
    "home loan", "mortgage",
    "personal loan",
    "wealth privilege", "privilege banking",
    "investment advisory", "portfolio management",
    "mutual fund", "sip",
    "tax optimization", "tax planning",
    "cymbalwealth", "cymbal wealth",
}


def _is_banking_product(product_name: str, specifications: str) -> bool:
    """Return True only if the product is a CymbalWealth banking/wealth offering."""
    combined = (product_name + " " + specifications).lower()
    return any(kw in combined for kw in _ALLOWED_CATEGORIES)
...
class ScriptService:
    ...
    async def generate_script(self, request: ScriptRequest) -> ScriptResponse:
        ...
        1. Validate the product is a CymbalWealth banking/wealth product
        2. Generate unique run_id
        3. Load product image (local path or HTTP download)
        4. Save product image locally
        5. Call Gemini to generate script
        6. Parse into VideoScript model
        7. Save script.json
        8. Return ScriptResponse
        """
        # ── CymbalWealth guardrail ──────────────────────────────────────────
        if not _is_banking_product(request.product_name, request.specifications or ""):
            raise HTTPException(
                status_code=422,
                detail=(
                    "CymbalWealth Ad Studio only generates ads for CymbalWealth "
                    "banking and wealth management products (e.g. Savings Account, "
                    "Credit Card, Fixed Deposit, Home Loan, Wealth Privilege). "
        ...
        raw_script = await self.gemini.generate_script(
            ...
            customer_profile=request.customer_profile.model_dump() if request.customer_profile else {},
        )
