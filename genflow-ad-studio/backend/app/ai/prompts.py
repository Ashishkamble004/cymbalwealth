...
def build_customer_profile_section(profile: dict) -> str:
    """Return a formatted CUSTOMER PROFILE block for the script prompt.

    Only includes fields that are non-empty so the prompt stays clean
    when the marketer hasn't filled in all optional fields.
    """
    lines: list[str] = []
    field_labels = [
        ...
        ("key_motivation",    "Key Motivation / Goal"),
    ...
    if not lines:
        return ""  # No profile provided — leave section out

    block = (
        "TARGET CUSTOMER PROFILE (personalise every element of the ad to this person):\n"
        + "\n".join(lines)
        + "\n\n"
        "PERSONALISATION RULES:\n"
        "- The avatar presenter must visually match the customer's demographic (age group, "
        "life stage). A pre-retirement customer should see a mature, distinguished presenter; "
        "a young professional should see a dynamic, aspirational presenter.\n"
        "- The dialogue language/tone must match the customer's language preference. "
        "If Hinglish, blend casual Hindi phrases naturally with English.\n"
        "- The pain point and CTA must speak directly to the customer's key motivation. "
        "A 'Home Ownership' motivated customer cares about EMI affordability and approval speed; "
        "a 'Legacy Planning' customer cares about stability and wealth transfer.\n"
        "- Location type influences visual backgrounds: Metro = glass tower offices and "
        "upscale cafes; Tier-2 = aspirational home interiors; Rural = warm community settings.\n"
        "- Income tier sets aspirational register: HNI/UHNI ads should feel exclusive and "
        "private; Mass Market ads should feel accessible, warm, and empowering.\n\n"
    )
    return block
...
SCRIPT_SYSTEM_INSTRUCTION = (
    ...
    "working exclusively for CymbalWealth — a premium wealth management and "
    "banking institution. You create short, cinematic 30-second video commercials "
    "that promote CymbalWealth's financial products and services ONLY. "
    "These include: Savings & Current Accounts, Fixed Deposits (FD), Recurring "
    "Deposits (RD), Credit Cards, Debit Cards, Home Loans, Personal Loans, "
    "Wealth Privilege programs, Investment Advisory, Portfolio Management, "
    "and Tax Optimization services.\n\n"
    "CONTENT GUARDRAILS (non-negotiable):\n"
    "- You MUST only create ads for CymbalWealth banking and wealth management "
    "products listed above.\n"
    "- You MUST REFUSE to generate scripts for non-financial products "
    "(e.g., consumer electronics, food, fashion, sports gear, pharmaceuticals, "
    "or any product unrelated to banking/finance/wealth management).\n"
    "- If the product is not a CymbalWealth financial product, return a JSON "
    'error: {"error": "Only CymbalWealth banking and wealth management products are supported."}\n\n'
    "BRAND VOICE:\n"
    "- Premium, trustworthy, and aspirational — never salesy or gimmicky.\n"
    "- Speak to the customer's financial goals: security, growth, legacy, privilege.\n"
    "- Always close with a clear CTA referencing CymbalWealth by name.\n\n"
...
video advertisement script for the following CymbalWealth product.
